...
export function SortableProjectCard({ id, project, isActive = true, onEdit, onResize }: SortableProjectCardProps) {
  const {
    attributes,
    listeners,
    setNodeRef,
  ...
  const handleResizePointerDown = (e: React.PointerEvent, dirX: number, dirY: number) => {
     ...
     // Drag X de ~40px = +1 Col. (Ici on lock sur X, le Y sera ignoré pour le calcul de shape)
     const dragSensitivityX = 40; 
     const rowSizeInPixels = 20;

     const handlePointerMove = (moveEvent: PointerEvent) => {
        const deltaX = moveEvent.clientX - startX;
        // On peut écouter deltaY si l'utilisateur tire plus sur la hauteur que la largeur, on prendra la distance dominante.
        const deltaY = moveEvent.clientY - startY;
        
        // On détermine le "déplacement principal". Si on attrape un coin, on regarde l'axe le plus tiré.
        ...
        const addCols = Math.max(distanceAxisX, distanceAxisY); // On prend la plus grande intention (élargir H ou V)
        
        // Limites strictes
        const newCols = Math.max(2, Math.min(24, currentCols + addCols));
        // Calcul verrouillé mathématiquement de la nouvelle hauteur correspondante (Lock-Ratio)
        const newRows = Math.round(newCols * rowUnitColMapping * ratioFactor);

        if (newCols !== scaleCols) {
           scaleCols = newCols;
           scaleRows = newRows;
           
           // Si on tire par la gauche ou le haut, on doit compenser le flux de CSS Grid.
           // ex: Je me suis élargi de 2 cols à gauche -> Ma div va être poussée de 80px à droite. Je dois setTranslate(-80)
           const diffCols = newCols - currentCols;
           const diffRows = newRows - currentRows;
           
           const offsetX = dirX === -1 ? -(diffCols * dragSensitivityX) : 0;
           const offsetY = dirY === -1 ? -(diffRows * rowSizeInPixels) : 0;
           
           setResizeOffset({ x: offsetX, y: offsetY });
           
           if (onResize) onResize(id, `col-span-${newCols} row-span-${newRows}`, false); 
        }
     };
     ...
     const handlePointerUp = (upEvent: PointerEvent) => {
        ...
        if (onResize) onResize(id, `col-span-${scaleCols} row-span-${scaleRows}`, true); 
     };
    ...
    >
      ...
      {onResize && (
        <>
          {/* Top Left */}
          <div 
             className={`absolute top-0 left-0 w-8 h-8 cursor-nwse-resize z-50 flex items-start justify-start p-1 transition-colors ${isResizing ? 'text-pink-500' : 'text-zinc-500 hover:text-white'}`}
             style={{ touchAction: 'none' }}
             onPointerDown={(e) => handleResizePointerDown(e, -1, -1)}
             title="Redimensionner la tuile"
          >
             <svg className="w-3 h-3 md:w-4 md:h-4 opacity-70 transform rotate-180" viewBox="0 0 24 24" fill="currentColor">
          ...
          {/* Top Right */}
          <div 
             className={`absolute top-0 right-0 w-8 h-8 cursor-nesw-resize z-50 flex items-start justify-end p-1 transition-colors ${isResizing ? 'text-pink-500' : 'text-zinc-500 hover:text-white'}`}
             style={{ touchAction: 'none' }}
             onPointerDown={(e) => handleResizePointerDown(e, 1, -1)}
             title="Redimensionner la tuile"
          >
             <svg className="w-3 h-3 md:w-4 md:h-4 opacity-70 transform -rotate-90" viewBox="0 0 24 24" fill="currentColor">
                <path d="M22 22H10L22 10zM22 6L6 22H2L22 2z" />
             </svg>
          </div>
          ...
          {/* Bottom Left */}
          <div 
             className={`absolute bottom-0 left-0 w-8 h-8 cursor-nesw-resize z-50 flex items-end justify-start p-1 transition-colors ${isResizing ? 'text-pink-500' : 'text-zinc-500 hover:text-white'}`}
             style={{ touchAction: 'none' }}
             onPointerDown={(e) => handleResizePointerDown(e, -1, 1)}
             title="Redimensionner la tuile"
          >
