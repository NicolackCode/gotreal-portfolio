...
function SortableItem({ project, onSpanChange, onEdit, onDelete }: { project: Project, onSpanChange: (id: string, val: string) => void, onEdit: (p: Project) => void, onDelete: (id: string, title: string) => void }) {
  const {
    attributes,
    listeners,
    setNodeRef,
  ...
  return (
    ...
    >
      <div className="flex items-center gap-4 flex-1">
        {/* Poignée de drag */}
        <div {...attributes} {...listeners} className="cursor-grab text-zinc-500 hover:text-white px-2">
          ☰
        </div>
        <div>
          <div className="flex items-center gap-2">
            <h3 className="text-sm font-bold uppercase tracking-wider text-white">{project.title}</h3>
            {getPriority(project.priority) === 1 && <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-yellow-500/20 text-yellow-500 border border-yellow-500/30">TOP 1</span>}
            {getPriority(project.priority) === 2 && <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-green-500/20 text-green-500 border border-green-500/30">TOP 2</span>}
            {getPriority(project.priority) === 3 && <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-blue-500/20 text-blue-500 border border-blue-500/30">TOP 3</span>}
            {getPriority(project.priority) === 4 && <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-orange-500/20 text-orange-500 border border-orange-500/30">RAW</span>}
          </div>
...
export default function GridEditor({ initialProjects, onUpdate, onEdit, onDelete }: GridEditorProps) {
  ...
  const handleSpanChange = (id: string, newSpan: string) => {
    setProjects(items => items.map(p => p.id === id ? { ...p, forced_span: newSpan } : p))
  }
  ...
  const categories = Array.from(new Set(projects.map(p => p.client || 'Sans Catégorie'))).sort()

  const displayedProjects = activeFilter === 'all' 
    ? projects
    : projects.filter(p => (p.client || 'Sans Catégorie') === activeFilter)
  ...
  return (
    <div className="bg-zinc-950 border border-zinc-800 p-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          <div className="flex items-center gap-4">
            <h2 className="text-lg uppercase tracking-wider text-white">Organisation de la Grille</h2>
            {isSaving && <span className="text-xs text-yellow-500 animate-pulse">Sauvegarde...</span>}
          </div>
          <p className="text-xs text-zinc-500 mt-1">Glissez-déposez pour réorganiser. Assignez des tailles pour sculpter le mur.</p>
        </div>
        <button 
          onClick={handleSave}
          disabled={isSaving}
